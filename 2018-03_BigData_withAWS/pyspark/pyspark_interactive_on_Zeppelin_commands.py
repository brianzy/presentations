...
%spark.pyspark
flightsDF = sqlContext.read.csv("s3n://leesa.east2.training/enriched_flight_data/ORD_flights_000", mode="DROPMALFORMED", inferSchema = True)
flightsDF = flightsDF.withColumnRenamed("_c0", "ORD_DELAY_ID") \
...
selected = prediction.withColumn("prob_of_cancel", secondelement(selected["probability"])).select("ID", "prob_of_cancel", "prediction",'label')
